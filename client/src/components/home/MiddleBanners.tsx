...
const MiddleBanners = () => {
        ...
        const loadBanners = async () => {
            try {
                ...
                const middleBanners = allBanners
                    .filter(b => b.position === 'HOME_MIDDLE' && b.isActive)
    ...
    // Auto-advance slider if more than 1 banner
    useEffect(() => {
    ...
    const BannerCard = ({ banner, className }: { banner: Banner, className?: string }) => (
        <motion.div
            ...
            className={`relative rounded-xl overflow-hidden cursor-pointer group h-[160px] sm:h-[200px] w-full shadow-md hover:shadow-xl transition-all duration-300 ${className}`}
        >
            <img
                src={banner.image}
                alt={banner.title}
    ...
    return (
        <section className="py-0 bg-gray-50">
            <div className="container-custom">
                ...
                {banners.length === 1 && (
                    <BannerCard banner={banners[0]} className="h-[200px] sm:h-[280px]" />
                )}

                {/* Case 2: More than 1 Banner */}
                {banners.length > 1 && (
                    <>
                        {/* Mobile: Slider */}
                        <div className="md:hidden relative h-[200px] overflow-hidden rounded-xl group/slider">
                        ...
                        {/* Desktop: Grid */}
                        <div className={`hidden md:grid gap-6 ${banners.length === 2 ? 'grid-cols-2' : 'grid-cols-2 lg:grid-cols-3'}`}>
                            {banners.map((banner) => (
                                <BannerCard key={banner.bannerId} banner={banner} className="h-[280px]" />
                            ))}
                        </div>
                    </>
                )}
